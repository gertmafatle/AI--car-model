#This is a self-driving car model
...
# Introducing last_x and last_y, used to keep the last point in memory when the sand is drawn on the map
last_x = 0
last_y = 0
n_points = 0 # the total number of points in the last drawing
length = 0 # the length of the last drawing

# Getting AI, and that contains neural network that represents Q-function
...
def init():
    global sand # sand is an array that has as many cells as graphic interface has pixels. Each cell has a one if there is sand, 0 otherwise.
    global goal_x # x-coordinate of the goal (where the car has to go)
    global goal_y # y-coordinate of the goal (where the car has to go)
...
class Game(Widget):
    ...
    def serve_car(self):y # starting the car when we launch the application
...
class MyPaintWidget(Widget):
    ...
    def on_touch_down(self, touch): # Left mouse button clicked to draw the sand
